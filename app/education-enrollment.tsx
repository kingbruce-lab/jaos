...
type Student = { id: string; student_no?: string; name: string; registration_date: string; gender: string; age: number; birth_date?: string | null; identity_masked: string; phone_masked: string; guardian_name?: string; guardian_phone_masked?: string; emergency_contact?: string; health_notes?: string; game: string; game_account?: string; current_rank?: string; course_period: string; study_start: string; study_end: string; accommodation_days: number; room_type: string; fee_notes: string; notes: string; learning_status?: string; receivable: string; received: string; cost: string; arrears: string; overpayment: string; version: number; fees: Fee[] };
...
function StudentForm({ student, options, staff, start, period, busy, onSubmit }: { student?: Student; options: EducationOptions; staff: Staff[]; start: string; period: string; busy: boolean; onSubmit: (event: FormEvent<HTMLFormElement>, fees: Fee[]) => void }) {
  ...
  return <form className="educationForm" onSubmit={(event) => onSubmit(event, fees)}>
    ...
    <label>课程周期<select name="course_period" defaultValue={student?.course_period || period}>{Object.entries(options.periods).map(([key, text]) => <option key={key} value={key}>{text}</option>)}</select></label>
    <label>学习开始日期<input name="study_start" type="date" required defaultValue={student?.study_start || start} /></label>
...
export function EducationEnrollment({ api, token, cohortId, start, period, canEdit, onSaved }: { api: Api; token: string; cohortId: string; start: string; period: string; canEdit: boolean; onSaved: () => void }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>, fees: Fee[]) {
  ...
  async function addStaff(event: FormEvent<HTMLFormElement>) {
    ...
    const values = Object.fromEntries(new FormData(event.currentTarget));
    setBusy(true); setError("");
    try { staffRequestId.current ||= crypto.randomUUID(); await api(`${base(cohortId)}/staff`, { method: "POST", body: JSON.stringify({ ...values, request_id: staffRequestId.current }), signal: AbortSignal.timeout(20000) }, token); staffRequestId.current = null; onSaved(); }
  ...
  return <section className="educationEnrollment">
    ...
    {options && <>
      <details className="educationRoster"><summary>教师 / 助教人员名册（{staff.filter((item) => item.active !== false).length}） · 管理人员</summary>
        <p className="educationHint">可添加或移除人员；移除不删除历史费用、评估记录，也不影响登录账号。</p>
        {staff.filter((item) => item.active !== false).map((item) => <p key={item.id} className="educationRosterPerson"><span>{item.name}（{item.role}）</span>{canEdit && <button type="button" disabled={busy} onClick={() => void removeStaff(item)}>移除人员</button>}</p>)}
        {!staff.some((item) => item.active !== false) && <p>暂无在册人员；此名册不创建登录账号。</p>}
        {canEdit && <form className="educationForm" onSubmit={(e) => void addStaff(e)}><label>人员姓名<input name="name" required maxLength={80} /></label><label>人员类型<select name="role">{options.staff_roles.map((role) => <option key={role}>{role}</option>)}</select></label><button className="primaryButton" disabled={busy}>添加人员</button></form>}
      </details>
      ...
      {canEdit && studentModule === "fees" && <details open={!!selected} key={selected?.id || "new"}><summary>{selected ? `修改报名：${selected.name}` : "＋ 新增学员报名"}</summary>
        ...
        <StudentForm key={selected ? `${selected.id}-${selected.version}` : "new"} student={selected || undefined} options={options} staff={staff} start={start} period={period} busy={busy} onSubmit={(event, fees) => void save(event, fees)} />
      </details>}
      {selected && !canEdit && <div className="educationStudentRead"><h4>{selected.name} · 报名明细</h4><p>{selected.student_no || "未生成学员编号"} · {selected.gender} · {selected.age}岁 · {selected.learning_status || "已报名"}</p><p>身份证 {selected.identity_masked || "未填写"} · 电话 {selected.phone_masked || "未填写"}</p><p>{selected.study_start} 至 {selected.study_end} · {selected.game} · {options.periods[selected.course_period]} · {selected.room_type} {selected.accommodation_days}天</p>{selected.fees.map((fee, index) => <p key={index}>{fee.category} / {fee.detail} · 应收 {money(fee.receivable)} · 成本 {money(fee.cost)} · {fee.note}</p>)}<p>费用备注：{selected.fee_notes}</p><p>备注：{selected.notes}</p></div>}
      <h4>学员列表</h4><div className="educationStudentList">{registry?.items.map((student) => <article key={student.id}>
        ...
        <span>学习：{student.study_start} 至 {student.study_end} · 住宿 {student.accommodation_days}天 · {student.room_type}</span>
